        ...
        <q-card class="text-center">
          <q-card-section>
            ...
            <div>
              ...
              <q-btn label="refresh" color="green" icon="refresh" class="q-ma-sm" />
            </div>
          </q-card-section>
          <q-card-section v-if="posts.length > 0">
            ...
            <q-item v-for="post in posts.filter(p => p.net_rshares > 0)" :key="post.index">
              <q-item-section>
...
export default {
  ...
  methods: {
    ...
    calcWeightForRshares (rsharesRequired) {
      ...
      var weight = 10000
      // var power = (this.account.voting_power * weight / 10000) / 500px
    ...
    async lookup (user) {
      this.username = user
      this.posts = []
      var params = { account: user, sort: 'payout' }
