  ...
  <div v-if="showTransactionDialog">
    <q-card flat bordered>
        ...
        <q-card-section v-if="loggedInUser && transaction.currencyFrom === 'hive' && detectedPayment === false && !['finished','sending'].includes(transaction.status)" class="text-center text-bold">
          ...
          <q-btn @click="transferNeededHive()" push icon="send" dense no-caps color="primary" v-if="!['finished','sending'].includes(transaction.status) && parseFloat(account.balance.split(' ')[0]) >= transaction.amountExpectedFrom && !disableTransferButton">Transfer {{ transaction.amountExpectedFrom }} HIVE to {{ transaction.payinAddress }} with memo {{ transaction.payinExtraId }}</q-btn>
        </q-card-section>
